...
function MMNotesData() {
	this.caseNotes = null;
	this.detectiveNotes = null;
	this.detectivePhone = null;
	this.submissionNotes = null;
}

function MMRecordData() {
...
function MMPartnerData() {
	this.caseWorkerEmail = null;
	this.caseWorkerName = null;
	this.caseWorkerOrg = null;
	this.caseWorkerPhone = null;
	this.partnerOrg = null;
}

function MMCaseDetail() {
	this.caseStatus = null;
	this.deliveryDate = null;
	this.deliveryStatus = null;
	this.FBLink = null;
	this.FBLinkDate = null;
	this.partnerInfo = new MMPartnerData();
	this.reunionDate = null;
	this.reunionStatus = null;
}

function MMVolunteerData() {
	this.volunteerName = null;
	this.volunteerContactInfo = null;
	this.caseManagerName = null;
	this.caseMarkedCold = null;
	this.linktoMM = null;
	this.messengerName = null;
	this.messengerEmail = null;
	this.messengerPhone = null;
}

function MMCaseData(caseid) {
	this.caseID = caseid;
	this.clientData = new MMClientData();
	this.loData = {};
	this.noteData = new MMNotesData();
	this.recordData = new MMRecordData();
	this.caseData = new MMCaseDetail();
	this.volunteerData = new MMVolunteerData();
}
...
function MakeCaseFromCsv(csvRec) {
	...
	try {
		...
		if (csvRec["C35"]) {
			var loData1 = new MMLovedOnesData();
			...
			if (csvRec["C42"]) {
				var loData2 = new MMLovedOnesData();
...
MMCaseData.prototype.loadFromDB = function (theDB) {
	theDB.ref("/cases/" + caseID).once('value').then(function (snapshot) { this.caseData = snapshot.val(); });
	theDB.ref("/clients/" + caseID).once('value').then(function (snapshot) { this.clientData = snapshot.val(); });
	theDB.ref("/loved_ones/" + caseID).once('value').then(function (snapshot) { this.loData = snapshot.val(); });
	theDB.ref("/records/" + caseID).once('value').then(function (snapshot) { this.recordData = snapshot.val(); });
	theDB.ref("/volunteers/" + caseID).once('value').then(function (snapshot) { this.volunteerData = snapshot.val(); });
	theDB.ref("/notes/" + caseID).once('value').then(function (snapshot) { this.noteData = snapshot.val(); });
}

MMCaseData.prototype.makeEditDialog = function () {
	// x instanceof Array
}
